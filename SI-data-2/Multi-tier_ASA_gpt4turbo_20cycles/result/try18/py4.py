...
def generate_unit_vector():
    ...
    theta = np.arccos(costheta)  # Correct usage of arccos
...
def compute_h2(chains):
    return np.mean([np.dot(chain[-1], chain[-1]) for chain in chains])
...
for N in Ns:
    ...
    # Plotting 50 randomly selected chains
...
# Estimating the scaling relationship v
v, _ = np.polyfit(np.log(Ns), np.log(h2_values), 1)
