...
def execute_script(pystr,N_py):
    ...
    with open("BIGpy"+str(N_py)+".py", "w", encoding='UTF-8') as f:
        f.write(pystr)

    # Execute py1.py script in the background, capturing output and errors
    process = subprocess.Popen(["python", "BIGpy"+str(N_py)+".py"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
